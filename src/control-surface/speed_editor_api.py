...
# Modified off of bmd.py from smunaut/blackmagic-misc
...
class SpeedEditorHandler:
  ...
  def jog(self, mode : SpeedEditorJogMode, value):
    """
    mode  [SpeedEditorJogMode]  Mode of the jog wheel
    value [int]                 Value of the jog wheel

    hint: in relative mode, divide `value` by ~1400000 to get number of revolutions
    """
    pass
...
class SpeedEditor:
  ...
  def __init__(self, handler):
    ...
    self.next_auth = self.authenticate() + time.time()
  ...
  def poll(self):
    ...
    if time.time() > self.next_auth:
      self.next_auth = self.authenticate() + time.time()

    # Get REPORT
    report = self.dev.read(64, timeout=int(self.next_auth - time.time()) * 1000)
    print(report)
    if len(report) == 0:
      return
